# Activations script for add the nocurses module to the lua path when
# building nocurses using the Makefile.
# Source this into interactive shell by invoking ". activates.sh" from this directory
# This is not necessary if nocurses is installed, e.g. via luarocks.

this_dir=$(pwd)

luanocurses_dir=$(cd "$this_dir"/..; pwd)

if [ ! -e "$luanocurses_dir/src/activate.sh" -o ! -e "$luanocurses_dir/src/main.c" ]; then

    echo '**** ERROR: ". activate.sh" must be invoked from "src" directory ***'

else

    echo "Setting lua paths for: $luanocurses_dir"

    add_lua_path="$luanocurses_dir/src/?.lua;$luanocurses_dir/src/?/init.lua"
    add_lua_cpath="$luanocurses_dir/src/build"
...
unset lua_cmd this_dir luanocurses_dir add_lua_path add_lua_cpath lua_version lua_path_vers vers default_version
